      ...
      <div className="relative aspect-square overflow-hidden bg-zinc-900">
        {showImage ? (
          ...
          <div className="flex h-full w-full flex-col items-center justify-center bg-gradient-to-br from-zinc-900 via-zinc-800 to-zinc-900 p-4">
            <span className="font-display text-5xl font-black text-gold/80 sm:text-6xl">
              {product.brand.charAt(0)}
            </span>
            <p className="mt-2 text-xs font-semibold uppercase tracking-widest text-gold/60">
              {product.brand}
            </p>
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-1">
            {product.tags.slice(0, 2).map((tag) => (
              <span
                key={tag}
                className="rounded-full bg-gold/10 px-1.5 py-0.5 text-[9px] font-medium text-gold sm:px-2 sm:text-[10px]"
              >
                {tag}
              </span>
            ))}
          </div>
          <span className="text-[10px] font-semibold text-gold group-hover:underline sm:text-xs">
            View Deal &rarr;
          </span>
        </div>
      </div>
